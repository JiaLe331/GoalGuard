...
export function GoalWorkspace({ goalId }: { goalId: string }) {
  ...
  const renderStage = () => {
    ...
    if (state.stage === "confirming_goal" && state.goal) return <GoalConfirmationForm key={state.goal.updatedAt} goal={state.goal} busy={busy} fieldErrors={state.error?.fieldErrors ?? {}} onSave={saveGoal} onFind={(value) => void findAndReview(value)} />;
