...
describe("deterministic council consensus", () => {
  ...
  it("surfaces the uncertain reviewer's concern as the disputed reason", () => { expect(councilConsensus(withVerdict(2, "uncertain", ["Settlement asset is unfamiliar."]))).toMatchObject({ status: "disputed", blockedReasons: ["Settlement asset is unfamiliar."] }); });
});
