...
export async function previewTrade(goalId: UUID, candidateId: UUID, decisionId: UUID, walletAddress: string, idempotencyKey: string, ownerSessionHash: string, repository = new PostgresGoalGuardRepository()): Promise<TradePreview> {
  ...
  if (goal.status !== "ready" || goal.selectedCandidateId !== candidate.id || goal.councilDecisionId !== decision.id) throw new ApiRouteError("CANDIDATE_STALE", "The selected candidate or council decision is no longer current.", 409, true);
  ...
  const savedTrade = await repository.createTrade(trade, { target: exact.execution.to, calldataHash: sha256(exact.execution.data.toLowerCase()), valueBaseUnits: exact.execution.valueBaseUnits, verificationDeadline: new Date(now.getTime() + 10 * 60_000).toISOString() }, ownerSessionHash);
...
export async function prepareExecution(tradeId: UUID, quoteFingerprint: string, walletAddress: string, ownerSessionHash: string, repository = new PostgresGoalGuardRepository()) {
  ...
  const [goal, candidate, decision] = await Promise.all([repository.getGoal(trade.goalId, ownerSessionHash), repository.getCandidate(trade.candidateId, ownerSessionHash), repository.getDecision(trade.councilDecisionId, ownerSessionHash)]);
  if (!goal || !candidate || !decision || goal.status !== "ready" || candidate.status !== "selected" || decision.status !== "approved" || goal.selectedCandidateId !== candidate.id || goal.councilDecisionId !== decision.id) throw new ApiRouteError("CANDIDATE_STALE", "The selected candidate or council decision is no longer current.", 409, true);
  const exact = await executionForCandidate(candidate, walletAddress, true);
